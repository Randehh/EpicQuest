...
public class CommandDrop {
	public static void Execute(CommandSender sender, Command command,
		...
		if (!CommandListener.hasPermission(ePlayer, "epicquest.user.drop"))
			return;
		...
		if (quest == -1 || quest > questList.size() - 1) {
			player.sendMessage(ChatColor.RED
					+ "That isn't a valid questbook number.");
			return;
		}
		...
		player.sendMessage(ChatColor.GREEN + "Quest '" + eQuest.getQuestName()
				+ "' succesfully dropped.");
	}
}
