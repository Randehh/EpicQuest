...
public class CommandParty {
	...
	public static void Execute(CommandSender sender, Command command,
		...
		if (!CommandListener.hasPermission(ePlayer, "epicquest.user.party"))
			return;
		...
		if (args[1].equalsIgnoreCase("accept")) {
			if (ePlayer.hasPartyInvitation != null
					&& invitationTimer.containsKey(ePlayer)) {
				...
				player.sendMessage(ChatColor.RED
						+ "You don't have a party invitation.");
			}
			return;
		}
		...
		if (args[1].equalsIgnoreCase("invite")) {
			...
			invitedPlayer.sendMessage(ChatColor.GREEN + player.getName()
					+ " invited you to his party (" + partySize + ").");
			invitedPlayer.sendMessage(ChatColor.GREEN
					+ "Type '/q party accept' to accept the invitation.");
			invitedEPlayer.hasPartyInvitation = ePlayer;

			invitationTimer.put(invitedEPlayer, 15);

			player.sendMessage("" + ChatColor.ITALIC + ChatColor.GREEN
					+ "You invited " + invitedPlayer.getName()
					+ " to your party.");
			return;
		}
