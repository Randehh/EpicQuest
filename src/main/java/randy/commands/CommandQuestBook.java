...
public class CommandQuestBook {
	public static void Execute(CommandSender sender, Command command,
		...
		if (!CommandListener.hasPermission(ePlayer, "epicquest.user.questbook"))
			return;

		if (args.length == 2 && args[1].equalsIgnoreCase("give")) {
			ePlayer.giveQuestBook();
			player.sendMessage(ChatColor.GREEN
					+ "You have been given a Questbook.");
			return;
		}
		...
		int maxpages = questList.size() / 10
				+ Math.min(1, questList.size() % 10);
		...
		pageString.append("  =======]\n");
		for (int quest = pageNumber * 10; quest < questList.size()
