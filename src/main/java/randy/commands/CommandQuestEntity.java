		...
		if (args[1].equalsIgnoreCase("create")) {
			...
			if (EpicSystem.useCitizens()) {
				...
				player.sendMessage(ChatColor.GREEN
						+ "Right click a Citizen to make it a quest giver.");
			} else {
				String name = "";
				...
				QuestEntityHandler.SpawnVillager(player.getWorld(),
						player.getLocation(), name);
				QuestEntityHandler.GetQuestEntity(
						QuestEntityHandler.GetEntity(player.getWorld(), name))
						.SetBasics(quest);

				player.sendMessage(ChatColor.GREEN
						+ "A villager with the name " + name
						+ " has been created with quest " + quest + ".");
			}
			...
			if (!QuestEntityHandler.RemoveVillager(player.getWorld(), name)) {
				player.sendMessage(ChatColor.RED + "A villager with the name "
						+ name + " has not been found in this world.");
			} else {
				player.sendMessage(ChatColor.GREEN
						+ "The villager with the name " + name
						+ " has been removed.");
			}
