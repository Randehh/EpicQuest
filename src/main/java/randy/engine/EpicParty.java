...
public class EpicParty {
	...
	public void addPlayer(EpicPlayer player) {
		if (player.getParty() == null) {
			...
			sendMessage(null, "" + ChatColor.ITALIC + ChatColor.GREEN
					+ player.getPlayer().getName() + " has joined the party!");
		}
	}
		...
		if (playerList.contains(player)) {
			...
			if (kicked)
				player.getPlayer().sendMessage(
						ChatColor.RED + "You were kicked out of the party by "
								+ partyLeader.getPlayer().getName() + ".");
			else
				sendMessage(null, "" + ChatColor.ITALIC + ChatColor.RED
						+ player.getPlayer().getName() + " has left the party.");
	...
	public void Disband() {
		for (int i = 0; i < playerList.size(); i++) {
			...
			epicPlayer.getPlayer().sendMessage(
	...
	public void setPartyLeader(EpicPlayer newLeader) {
		...
		sendMessage(null, "" + ChatColor.ITALIC + ChatColor.GREEN
				+ newLeader.getPlayer().getName() + " is now party leader.");
	}
	...
	public void sendPartyMembers(EpicPlayer originalPlayer) {
		...
		player.sendMessage(ChatColor.ITALIC + partyLeader.getPlayer().getName()
				+ ChatColor.RESET + ChatColor.GREEN + " (Leader)");
	...
	public void sendMessage(EpicPlayer epicPlayer, String message) {
		if (epicPlayer != null) {
			for (int i = 0; i < playerList.size(); i++) {
				String playerTag = "";
				if (epicPlayer == partyLeader)
					playerTag = "(LDR)";
				playerList
						.get(i)
						.getPlayer()
						.sendMessage(
								ChatColor.ITALIC + "" + ChatColor.GREEN
										+ playerTag + ChatColor.WHITE
										+ epicPlayer.getPlayer().getName()
										+ ": " + message);
			}
		} else {
			for (int i = 0; i < playerList.size(); i++) {
	...
	public List<EpicPlayer> getClosePlayers(EpicPlayer originalPlayer,
			float proximity) {
		List<EpicPlayer> closePlayers = new ArrayList<EpicPlayer>();
		Location originalPlayerLocation = originalPlayer.getPlayer()
				.getLocation();
		for (int i = 0; i < playerList.size(); i++) {
			EpicPlayer player = playerList.get(i);
			if (player.getPlayer().getLocation()
					.distance(originalPlayerLocation) < proximity) {
