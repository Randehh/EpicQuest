...
public class EpicPlayer {
	...
	public EpicPlayer(UUID playerID, List<EpicQuest> questList,
			List<String> questCompleted, int questDailyLeft,
	...
	public boolean hasPermission(String permission) {
		...
		if (permission.contains(".user."))
			return true;
		if (permission.contains(".admin.") && getPlayer().isOp())
			return true;
		return false;
	}
	...
	public EpicQuest getQuest(int questNo) {
		return questList.get(questNo);
	}
	public EpicQuest getQuestByTag(String questTag) {
		for (int i = 0; i < questList.size(); i++)
			if (getQuest(i).getQuestTag().equals(questTag))
				return getQuest(i);
		return null;
	}
	...
	public int getQuestDailyLeft() {
		return questDailyLeft;
	}
	public HashMap<String, Integer> getQuestTimerMap() {
		return questTimer;
	}
	public List<EpicQuestTask> getTasksByType(TaskTypes type) {
		if (questTasks.containsKey(type))
			return questTasks.get(type);
		return new ArrayList<EpicQuestTask>();
	}

	public void setQuestsCompleted(List<String> newList) {
		questCompleted = newList;
	}
	public void setQuestTimerMap(HashMap<String, Integer> newMap) {
		questTimer = newMap;
	}
	public void setQuestTimer(String quest, Integer time) {
		questTimer.put(quest, time);
	}
	public void setQuestDailyLeft(int questsLeft) {
		questDailyLeft = questsLeft;
	}
	...
	public boolean addQuest(EpicQuest quest) {
		if (canGetQuest(quest.getQuestTag()) == true) {
			...
			if (hasItemRequirements(quest.getQuestTag())) {
				List<ItemStack> itemList = EpicQuestDatabase
						.getQuestItemsRequired(quest.getQuestTag());
				Inventory inventory = getPlayer().getInventory();
			...
			for (int i = 0; i < quest.getTasks().size(); i++) {
				getPlayer().sendMessage(
						quest.getTasks().get(i).getPlayerTaskProgressText());
			}
			...
			if (!hasItemRequirements(quest.getQuestTag())) {
				getPlayer()
						.sendMessage(
								ChatColor.RED
										+ "You do not have the required items for this quest!");
			}
			return false;
		}
	}
	...
	public void completeAllQuests() {
		...
		if (!completeableQuests.isEmpty()) {
			...
			getPlayer().sendMessage(
					ChatColor.RED + "There are no quests to complete!");
		}
	}
	...
	public void addQuestRandom() {
		...
		if (!obtainableQuests.isEmpty()) {
			...
			EpicQuest quest = new EpicQuest(this,
					obtainableQuests.get(randomQuest));
			addQuest(quest);
		}
	}
	...
	public void addTask(EpicQuestTask task) {
		...
		if (!questTasks.containsKey(type))
			questTasks.put(type,
					(List<EpicQuestTask>) new ArrayList<EpicQuestTask>());
		if (!questTasks.get(type).contains(task))
			questTasks.get(type).add(task);
	}
	...
	public boolean canGetQuest(String questTag) {
		...
		if (!hasQuest(questTag) && hasUnlockedQuest(questTag)
				&& hasDailyQuestLeft() && isTimeOut(questTag)
				&& !isQuestListFull() && hasItemRequirements(questTag)
	...
	public boolean canGetQuest() {
		if (getObtainableQuests().isEmpty()) {
			return false;
		} else {
			return true;
		}
	...
	public boolean hasUnlockedQuest(String questTag) {
		List<String> questLockedList = EpicQuestDatabase
				.getQuestLocked(questTag);
		if (questLockedList.isEmpty())
			return true;

		if (questCompleted.containsAll(questLockedList)) {
			return true;
		}
		return false;
	}
	...
	public boolean isTimeOut(String questTag) {
		if (EpicQuestDatabase.getQuestResetTime(questTag) == -1
				&& getQuestsCompleted().contains(questTag))
			return true;

		checkTimer(questTag, false);

		if (questTimer.get(questTag) <= 0) {
			return true;
		}
		return false;
	}
	public boolean isQuestListFull() {
		if (questList.size() >= EpicSystem.getQuestLimit()) {
			return true;
		}
		return false;
	}
	public boolean hasItemRequirements(String questTag) {
		List<ItemStack> itemList = EpicQuestDatabase
				.getQuestItemsRequired(questTag);
		Inventory inventory = getPlayer().getInventory();
