...
public class QuestEntity {
	...
	public void NextInteraction(EpicPlayer epicPlayer) {
		// Just in case somehow the players doesn't have a quest, assume the
		// player didn't talk to the NPC yet
		if (!currentQuest.containsKey(epicPlayer)
				|| (currentQuest.containsKey(epicPlayer) && currentQuest
						.get(epicPlayer) == null))
			currentQuest.put(epicPlayer, questList.get(0));
		...
		// epicPlayer.getPlayer().sendMessage("Player info. Quest: " +
		// currentQuest + ", Phase: " + currentPhase.toString());
		boolean loop = true;
		while (loop) {
			switch (currentPhase) {
				case INTRO_TALK :
					...
					epicPlayer.getPlayer().sendMessage(
							formatMessage(sentences.Next(epicPlayer)));
					if (sentences.IsLast(epicPlayer)) {
						epicPlayer.addQuest(new EpicQuest(epicPlayer,
								currentQuest));
				...
				case BUSY :
					...
					if (!epicPlayer.getQuestByTag(currentQuest).isCompleted()) {
						...
						epicPlayer.getPlayer().sendMessage(
								formatMessage(sentences.Random(epicPlayer)));
						loop = false;
						break;
					}
				...
				case ENDING_TALK :
					...
					epicPlayer.getPlayer().sendMessage(
							formatMessage(sentences.Next(epicPlayer)));
					if (sentences.IsLast(epicPlayer)) {
						epicPlayer.completeQuest(epicPlayer
								.getQuestByTag(currentQuest));
						questPhases.put(epicPlayer, QuestPhase.INTRO_TALK);
					}
					loop = false;
					break;
