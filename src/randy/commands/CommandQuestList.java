...
public class CommandQuestList {
	public static void Execute(CommandSender sender, Command command, String commandName, String[] args){
		...
		if(!CommandListener.hasPermission(ePlayer, "epicquest.user.questlist")) return;
			...
			int maxpages = availableQuests.size() / 10 + Math.min(1, availableQuests.size() % 10);
			...
			pageString.append("  =======]\n");
			for(int quest = pageNumber * 10; quest < availableQuests.size() && quest < pageNumber * 10; quest++){
				StringBuilder questString =  new StringBuilder();
				questString.append(ChatColor.GREEN);
				questString.append(quest);
				questString.append(": ");
				questString.append(EpicQuestDatabase.getQuestName(availableQuests.get(quest)));
				questString.append("\n");
