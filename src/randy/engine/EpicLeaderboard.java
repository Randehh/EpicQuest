...
public class EpicLeaderboard {
	...
	private static List<String> getTopScores(final HashMap<UUID, Float> map){
		...
		if(map.isEmpty()) return scoreList;
		...
		for(UUID id : idList){
			scoreList.add(Bukkit.getOfflinePlayer(id).getName() + ": " + map.get(id).intValue());
		}
		return scoreList;
	}
	...
	public static void setTopQuestsCompletedScore(EpicPlayer ePlayer){
		Float playerScore = Float.valueOf(ePlayer.playerStatistics.GetQuestsCompleted());
	...
	public static void setTopTasksCompletedScore(EpicPlayer ePlayer){
		Float playerScore = Float.valueOf(ePlayer.playerStatistics.GetTasksCompleted());
	...
	public static void setTopMoneyEarned(EpicPlayer ePlayer){
		Float playerScore = Float.valueOf(ePlayer.playerStatistics.GetMoneyEarned());
